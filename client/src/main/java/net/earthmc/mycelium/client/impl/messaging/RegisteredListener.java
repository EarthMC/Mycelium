...
public record RegisteredListener<T>(MessagingRegistrarImpl registrar, BoundChannelIdentifier<T> identifier, String canonicalChannel, Consumer<IncomingMessage<T>> consumer) implements Listener {
    ...
    public void processIncoming(MyceliumClient client, InternalMessage incoming, MessageSender sender) {
        try {
            final T deserialized = identifier.gson().fromJson(incoming.payload, identifier.codec().type());

            this.consumer.accept(new IncomingMessageImpl<>(client, incoming, sender, this.identifier().codec(), deserialized));
